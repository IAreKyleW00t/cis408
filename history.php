<?php
    ...
    $query = $sql->prepare('SELECT id, order_number, account, SUM(quantity) AS quantity, SUM(price) AS price, purchase_time FROM order_history WHERE account = ? GROUP BY order_number');
    $query->execute(array($_SESSION['user']['id']));
    
    $orders = $query->fetchAll(PDO::FETCH_ASSOC);
    
    $query = $sql->prepare('SELECT o.order_number AS order_number, p.name AS name FROM order_history o, products p WHERE o.account = ? AND o.item = p.id');
    $query->execute(array($_SESSION['user']['id']));
    
    $items = $query->fetchAll(PDO::FETCH_COLUMN|PDO::FETCH_GROUP);
...
<!--[if gt IE 8]><!--><html class="no-js" lang="en"><!--<![endif]-->
    <head>
        ...
        <title>CSUOhio Shopping :: Order History</title>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <table class="table table-stripped table-hover">
                                ...
                                <tbody>
                                    <?php
                                        foreach ($orders as $o) {
                                            $list = "";
                                            foreach ($items[$o['order_number']] as $i) {
                                                $list .= $i . ', ';
                                            }
                                            echo '<tr id="' . $o['id'] . '">';
                                            echo '<td>' . date('F j, Y (g:iA)', strtotime($o['purchase_time'])) . '</td>';
                                            echo '<td>' . $o['order_number'] . '</td>';
                                            echo '<td>' . rtrim($list, ', ') . '</td>';
                            ...
                            <?php else : ?>
                            <h3 class="text-center">No history</h3>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
